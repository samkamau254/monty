...
/**
 * parse_byte_code - reads and interprets Monty bytecode files
...
void parse_byte_code(int argc, char *argv[])
{
	...
	start(file);
	bytesRead = get_line(&globals.line, &lineLength, file);
	while (bytesRead != -1)
	{
